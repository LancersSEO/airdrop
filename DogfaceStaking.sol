...
contract DogfaceStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    uint256 public rate = 4760000000000000; 
    uint256 public gen1pass = 10000; 
    uint256 public gen2pass = 5000; 
    uint256 public gen3pass = 1000; 
...
/* STAKING MECHANICS */
    ...
    // Set a multiplier for how many tokens to earn each time a block passes. 
        // 30 $AMMO PER DAY
        // n Blocks per day = 6300, Token Decimal = 18
        // Rate = 4760000000000000           
    ...
    function calculateReward(address account, uint256 tokenId) public view returns (uint256) {
      ...
      if (tokenId < 2445) {
          if (tokenId < 301) {
            onetimemint = gen1pass;
          } else {
            onetimemint = gen2pass;
          }
      } else {
            onetimemint = gen3pass;
      }

      return (_deposits[account].contains(tokenId) ? onetimemint : 0) + (rate * (_deposits[account].contains(tokenId) ? 1 : 0) * (Math.min(block.number, expiration) - _depositBlocks[account][tokenId]));
    }
